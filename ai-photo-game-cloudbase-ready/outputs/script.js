(function () {
  ...
  function shouldRevealAnswers() {
    return hasSubmitted || savedAnswerExists;
  }

  function openPreview(image) {
    const truthLabel = shouldRevealAnswers() ? truthNames[image.truth] : null;
    clearTimeout(previewCloseTimer);
    previewImage.src = image.src;
    previewImage.alt = `题目图片 ${image.label} 放大预览`;
    previewCaption.textContent = truthLabel ? `SAMPLE ${image.label} · ${truthLabel.main}` : `SAMPLE ${image.label}`;
  ...
  function renderCards() {
    ...
    currentQuestion.images.forEach((image, index) => {
      ...
      const revealDetails = shouldRevealAnswers();
      ...
      card.innerHTML = `
        <div class="sample-media">
          ...
          <span class="scan-status">${revealDetails ? "VERIFIED" : "UNVERIFIED"}</span>
        </div>
        <div class="card-body">
  ...
  async function fetchQuestionByDate(date) {
    try {
      const response = await fetch(`${apiUrl("/get-question-by-date")}?date=${encodeURIComponent(date)}`);
  ...
  async function fetchTodayQuestion() {
    try {
      const response = await fetch(`${apiUrl("/get-today-question")}?date=${encodeURIComponent(today)}`);
  ...
  async function loadTodayQuestion() {
    ...
    try {
      ...
      showEmptyState("题目加载失败", error.message || "请稍后再试，或检查 CloudBase 云函数配置。");
      return;
    }
  }
