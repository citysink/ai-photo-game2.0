...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (payloadError) return json(400, { ok: false, error: payloadError });
    ...
    if (existing) {
      return json(409, { ok: false, error: "这个日期已经发布过题目。请换一个日期，或先在 Supabase 中处理旧题。" });
    }
    ...
    for (const [index, image] of payload.images.entries()) {
      ...
      if (!decoded || decoded.mimeType !== image.file.type) {
        return json(400, { ok: false, error: `图片 ${image.label} 内容格式不正确。` });
      }
      if (decoded.buffer.length > maxFileSize) {
        return json(400, { ok: false, error: `图片 ${image.label} 实际大小超过 1.5MB，请先压缩。` });
      }
