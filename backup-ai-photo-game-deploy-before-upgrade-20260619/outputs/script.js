(function () {
  ...
  function getStorageKey(question) {
    return `ai-photo-game:${question.id || question.date}`;
  }

  function getLegacyStorageKey(question) {
    return `ai-photo-game:${question.date}:${question.title}`;
  }
  ...
  function openPreview(image) {
    ...
    previewCaption.textContent = `图片 ${image.label}`;
    ...
    currentQuestion.images.forEach((image) => {
      ...
      card.innerHTML = `
        ...
        <div class="card-body">
          ...
          <span class="truth-chip ${image.truth === "real" ? "truth-real" : "truth-ai"}">${truthNames[image.truth]}</span>
  ...
  function showResult(savedResult) {
    ...
    resultSummary.textContent = didWin ? "答对了！今天的观察力在线。" : "答错了！AI 今天小胜一局。";
    answerList.innerHTML = `
      <strong>正确答案：</strong>${correctLabels}<br />
      <span>每张图片下方已标出真实照片或 AI 生成图，并附上简短解析。</span>
    `;
  }
  ...
  function loadSavedResult(question) {
    const raw = localStorage.getItem(getStorageKey(question)) || localStorage.getItem(getLegacyStorageKey(question));
    if (!raw) return null;
  ...
  async function fetchQuestionByDate(date) {
    try {
      const response = await fetch(`/.netlify/functions/get-question-by-date?date=${encodeURIComponent(date)}`);
  ...
  async function fetchTodayQuestion() {
    try {
      const response = await fetch(`/.netlify/functions/get-today-question?date=${encodeURIComponent(today)}`);
