...
export default async function handler(request) {
  ...
  try {
    if (request.method === "GET" && (path === "/api/today" || path === "/api/get-today-question")) {
      ...
      return getQuestionResponse(date);
    }
    ...
    if (request.method === "DELETE" && path === "/api/admin/today") {
      ...
      const date = getDateParam(url) || todayInShanghai();
      return deleteQuestion({ adminPassword, date });
    }
...
async function getQuestionResponse(date) {
  const dateError = validateDate(date);
  if (dateError) return json(400, { ok: false, error: dateError });

  const question = await getQuestionsStore().get(questionKey(date), { type: "json" });
  if (!question) {
    return json(200, {
      ok: true,
      found: false,
...
async function publishQuestion(request) {
  const payload = await readJson(request);
  const validationError = validateAdminPassword(payload.adminPassword) || validateQuestionPayload(payload);
  ...
  await store.setJSON(questionKey(question.date), question);

  const savedQuestion = await store.get(questionKey(question.date), { type: "json" });
  ...
  if (imageCount < MIN_IMAGES || imageCount > MAX_IMAGES) {
    await store.delete(questionKey(question.date));
    return json(500, {
      ok: false,
...
async function deleteQuestion({ adminPassword, date }) {
  const validationError = validateAdminPassword(adminPassword) || validateDate(date);
  if (validationError) return json(400, { ok: false, error: validationError });
...
function getQuestionsStore() {
  return getStore({ name: "questions", consistency: "strong" });
}
...
function validateImageData(src, label) {
  if (!src.startsWith("data:")) return "Netlify Blobs 版本需要前端提交压缩后的图片 data URL";
...
async function readJson(request) {
  ...
  try {
    ...
    const formatError = new Error(`请求 JSON 格式不正确：${error.message}`);
...
function validateAdminPassword(adminPassword) {
  ...
  if (!expectedPassword) return "Netlify 环境变量 ADMIN_PASSWORD 未配置";
  if (adminPassword !== expectedPassword) return "管理员密码不正确。";
  return "";
}
