...
4. 填写标题、说明、题型。
5. 拖拽或点击上传 4 到 6 张图片。
6. 每张图选择真实属性并填写解析。AI生成图会自动成为正确答案。
7. 如需重发同一天题目，先点击“删除该日期题目”，确认后再重新发布。
8. 点击“发布今日题目”。
9. 打开 `/index.html`，如果日期是今天，就会显示刚发布的题目。
...
- 图片是否超过限制，尤其是压缩后单张 1.5MB 或总大小 4MB。
- 题目日期是否已经发布过；需要重发时先在管理员页删除该日期题目。
- 如果删除题目后 Storage 中仍残留旧图片，请检查 `question_images.storage_path` 字段是否已完成迁移。旧数据没有 storage_path 时系统会尝试从 image_url 解析，但少数旧 URL 可能无法完全清理。
